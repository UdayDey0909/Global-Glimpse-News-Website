...
async function fetchHealthNews() {
  try {
    const apiUrl = `${BASE_URL}/everything?q=health&searchIn=title,description&page=${page}&pageSize=8&apikey=${apikey}`;
    ...
    if (totalResultsElement) {
      totalResultsElement.textContent = `Total Health Articles: ${totalResults}`;
    }
...
function displayHealthNews(articles) {
  articles.forEach((article) => {
    ...
    if (scrollAnchor && healthNews.contains(scrollAnchor)) {
      healthNews.insertBefore(blogCard, scrollAnchor);
    } else {
      healthNews.appendChild(blogCard);
    }
  });
}

//? Fetch & Display Health News Cards

(async () => {
  try {
    const articles = await fetchHealthNews();
    displayHealthNews(articles);
  } catch (error) {
    console.error("Error fetching Health News:", error);
  }
})();
